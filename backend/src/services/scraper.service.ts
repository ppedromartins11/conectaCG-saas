...
/**
 * Scraper ético: só busca páginas públicas, respeita robots.txt,
 * delay entre requests, identifica-se via User-Agent, não armazena dados pessoais.
 *
 * Estratégia: busca planos disponíveis publicamente nos sites das operadoras
 * que atendem Campo Grande – MS.
 */
...
export async function runScraper() {
  logger.info('[Scraper] Starting ethical data ingestion...')

  const city = await prisma.city.findFirst({ where: { slug: 'campo-grande', isActive: true } })
  if (!city) {
    logger.warn('[Scraper] Campo Grande city not found or inactive')
    return
  }
  ...
  for (const source of OPERATOR_SOURCES) {
    try {
      ...
      const provider = await prisma.provider.findUnique({ where: { slug: source.name } })
      if (!provider) {
        logger.warn(`[Scraper] Provider ${source.name} not found in DB, skipping`)
        continue
      }
      ...
      if (!robotsAllowed) {
        logger.info(`[Scraper] robots.txt disallows scraping ${source.url}`)
        continue
      }
      ...
      for (const plan of plans) {
        ...
        if (existing) {
          ...
          if (existing.price !== plan.price) {
            await prisma.priceSnapshot.create({ data: { planId: existing.id, price: existing.price } })
      ...
      logger.info(`[Scraper] ${source.name}: processed ${plans.length} plans`)
    } catch (err: any) {
      logger.error(`[Scraper] Error processing ${source.name}: ${err.message}`)
    }
  }

  logger.info(`[Scraper] Done. Updated/created ${totalUpdated} plans`)
  return totalUpdated
}

async function checkRobotsTxt(siteUrl: string): Promise<boolean> {
  try {
    ...
    if (!res.ok) return true // If no robots.txt, assume allowed
 ...
 */
async function fetchPublicPlans(url: string, hint: string): Promise<ScrapedPlan[]> {
  try {
    ...
    logger.info(`[Scraper] ${hint}: HTML page detected - manual review needed for parsing`)
    return []
  } catch {
    return []
  }
...
function extractSpeed(text: string): { download: number; upload: number } | null {
...
function inferCategories(downloadSpeed: number): string[] {
  const cats: string[] = []
  if (downloadSpeed >= 300) cats.push('Gaming')
  if (downloadSpeed >= 100) cats.push('Streaming')
  cats.push('Trabalho')
  return cats
}
